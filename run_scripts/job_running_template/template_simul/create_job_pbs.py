...
def extract_number_of_nodes(specfemdir):
...
def modify_job_sbatch_file(inputfile, outputfile,
...
def setup_job_scripts(config):
    ...
    nnodes_per_simulation = extract_number_of_nodes(specfemdir)
    nnodes_per_job = nnodes_per_simulation * nevents_per_mpirun

    # calcuate walltime
    hour, minute = divmod(walltime_per_simulation * (len(deriv_cmt_list) + 1),
                          60)
    walltime = "%d:%02d:00" % (hour, minute)

    eventlist = read_txt_into_list(eventlist_file)
    if len(eventlist) > nevents_per_mpirun:
        raise ValueError("XEVENTID file larger than nevents_per_mpirun:"
                         "%d, %d" % (len(eventlist), nevents_per_mpirun))

    print("====== Create job scripts =======")
    print("Number of events in simul run:", nevents_per_mpirun)
    ...
    specfemdir = os.path.join(runbase, "specfem3d_globe")
    modify_job_sbatch_file(job_template, outputfn,
                           nnodes_per_job, walltime, deriv_cmt_list,
                           specfemdir)
    # os.remove(job_template)

    # modify parfile to simulataneous run
    modify_parfile_for_simul_run(specfemdir, nevents_per_mpirun)
